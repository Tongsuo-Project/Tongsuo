...
static const char *short_msg = "message digest";
...
int main(void)
{
    ...
    for (; i < ITR_NUM; i++) {
        ...
        *p = OSSL_PARAM_construct_end();
        ...
        EVP_PKEY_set_alias_type(sm2_key, EVP_PKEY_SM2);
        if ((mctx = EVP_MD_CTX_new()) == NULL) {
            goto err;
        }
        if (!EVP_PKEY_CTX_set1_id(sm2_ctx, userid, uidlen)) {
            goto err;
        }
        EVP_MD_CTX_set_pkey_ctx(mctx, sm2_ctx);
        if (!EVP_DigestSignInit(mctx, NULL, EVP_sm3(), NULL, sm2_key)) {
            goto err;
        }
        if (!EVP_DigestSign(mctx, NULL, &siglen,
                            (const unsigned char *)short_msg, inlen)) {
            goto err;
        }
        ...
        if (!EVP_DigestSign(mctx, sig, &siglen,
                            (const unsigned char *)short_msg, inlen)) {
            goto err;
        }
        end = get_time();
        ...
        /* it's not possible this will fail since we have just done the signing test */
        /* don't omit the NULL check in real world program */
        if (!EC_KEY_set_public_key(ec_key,
                                   EC_KEY_get0_public_key(EVP_PKEY_get0_EC_KEY(sm2_key)))) {
            goto err;
        }
        EVP_PKEY_assign_EC_KEY(sm2_key_pub, ec_key);
        EVP_PKEY_set_alias_type(sm2_key_pub, EVP_PKEY_SM2);
        /* SM2 verify */
        start = get_time();
        sm2_ctx = EVP_PKEY_CTX_new(sm2_key_pub, NULL);
        if (sm2_ctx == NULL) {
            goto err;
        }
        if ((mctx = EVP_MD_CTX_new()) == NULL) {
        ...
        /*
         * no need to free ec_key since it's assigned to sm2_key_pub, that
         * means it will be freed along with the parent
         */
        EVP_PKEY_free(sm2_key_pub);
        EVP_MD_CTX_free(mctx);
        inlen = RND_DATA_SIZE;
