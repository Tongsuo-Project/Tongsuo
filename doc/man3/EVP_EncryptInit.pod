...
EVP_get_cipherbyname() will return NULL for algorithms such as "AES-128-SIV" and
"AES-128-CBC-CTS" which were previously only accessible via low level interfaces.
...
block size. (The block size for AES is 16 bytes).
...
using a "AES-XXX-CBC" cipher otherwise the second last
cipher text block is a partial block.

=item "CS2"

For input lengths that are multiples of the block size it is equivalent to
using a "AES-XXX-CBC" cipher, otherwise it is the same as
"CS3" mode.
...
then this is equivalent to using a "AES-XXX-CBC" cipher.
