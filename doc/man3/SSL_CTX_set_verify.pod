...
SSL_CTX_set_verify_cert_with_sni,
SSL_CTX_get_verify_cert_with_sni
...
SSL_CTX_set_verify_cert_with_sni() sets the flag B<value> for whether to verify
the matching of the certificate and server_name during the ssl handshake for
B<ctx>. 1 is to verify, 0 is not to verify.

SSL_CTX_get_verify_cert_with_sni() gets the flag of B<ctx> that whether to
verify the matching of the certificate and server_name during the ssl handshake.
1 is to verify, 0 is not to verify.
...
SSL_CTX_set_verify_cert_with_sni has no return value.

The SSL_CTX_get_verify_cert_with_sni() function returns 1 or 0.
