...
A new curve can be constructed by calling EC_GROUP_new() or EC_GROUP_new_ex(),
using the implementation provided by B<meth> (see L<EC_GFp_simple_method(3)>).
If necessary, call EC_GROUP_new_ex() to bind B<engine> to a newly created
EC_GROUP, or use EC_GROUP_set_engine() to bind B<engine> to an existing EC_GROUP.
