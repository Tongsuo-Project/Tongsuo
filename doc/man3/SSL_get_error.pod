...
=head1 SYNOPSIS
 ...
 #define SSL_get_desc_and_level SSL_get_alert_level
...
SSL_get_alert_level() gets the level in the alert protocol message, and writes
the result if the parameters I<level> pointer is not I<NULL>. If the alter level
is one of B<SSL3_AL_WARNING> and B<SSL3_AL_FATAL>, the parameter I<desc> will be
written warn_alert or fatal_alert code.
...
SSL_get_alert_level() return 1 on success, or 0 on error.

The following return values for SSL_get_error() can currently occur:
