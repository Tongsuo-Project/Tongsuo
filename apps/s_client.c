...
#endif
#ifndef OPENSSL_NO_DELEGATED_CREDENTIAL
    {"dc", OPT_DELEGATED_CREDENTIAL, 's', "delegated credential path"},
    {"dc_pkey", OPT_DELEGATED_CREDENTIAL_PRIVATE_KEY, 's', "dc private key path"},
    {"enable_verify_peer_by_dc", OPT_ENABLE_VERIFY_PEER_BY_DC, '-', "enable verify peer by delegated credential"},
    {"enable_sign_by_dc", OPT_ENABLE_SIGN_BY_DC, '-', "enable sign by delegated credential"},
#endif
...
# endif
 ...
 opthelp:
    ...
    if (enable_sign_by_dc) {
        dc = DC_load_from_file_ex(s_delegated_credential_file,
