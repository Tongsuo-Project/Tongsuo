...
static int ssl_do_config(SSL *s, SSL_CTX *ctx, const char *name, int system)
{
    ...
    for (i = 0; i < cmd_count; i++) {
     ...
     && (!defined OPENSSL_NO_SM3) && (!defined OPENSSL_NO_SM4)
         ...
         * some test case can't cover previous scenarios, so I think this way
         * maybe a better way to solve the problem

        if (strcmp(cmdstr, "MaxProtocol") == 0
            && strcmp(arg,"SSLv3") == 0)
            arg = "MIN_VERSION_WITH_NTLS";
#endif*/
