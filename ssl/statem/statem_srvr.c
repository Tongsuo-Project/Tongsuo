...
MSG_PROCESS_RETURN tls_process_client_hello(SSL *s, PACKET *pkt)
{
    ...
    p = (unsigned char *)clienthello->ciphersuites.curr;
    if (s->status_param.ssl_status_enable) {
        ...
        if (s->status_callback(p, clienthello->ciphersuites.remaining,
                                   &s->status_param) == -1) {
            /* TODO: debug info */
            goto err;
        }
...
static int tls_early_post_process_client_hello(SSL *s)
{
    ...
    if (s->ctx->client_hello_cb != NULL) {
        ...
        /* parse ocsp status here or dycert ocsp will be disable */
        if (!tls_parse_extension(s, TLSEXT_IDX_status_request,
...
#if !(defined(OPENSSL_NO_GLOBAL_SESSION_CACHE) && defined(OPENSSL_NO_SESSION_LOOKUP))
 query_session_reentry:
        ...
        if (i == 1) {
            ...
            s->global_session_state = SSL_SESS_STAT_WAITING_GET;
            s->clienthello->ciphers = ciphers;
            /* any value bigger than 0 is fine; less than 0 means continuing working */
            return 1;
#endif
#ifndef OPENSSL_NO_SESSION_LOOKUP
        } else if (i == -3) {
...
WORK_STATE tls_post_process_client_hello(SSL *s, WORK_STATE wst)
{
    ...
    if (wst == WORK_MORE_A) {
        ...
        if (rv < 0)
            return WORK_MORE_A;
#ifndef OPENSSL_NO_GLOBAL_SESSION_CACHE
        if (s->global_session_state == SSL_SESS_STAT_WAITING_GET)
            return WORK_FINISHED_PAUSE_GET_SESSION;
#endif
        wst = WORK_MORE_B;
    }
    if (wst == WORK_MORE_B) {
...
MSG_PROCESS_RETURN tls_process_client_compressed_certificate(SSL *s,
                                                             PACKET *pkt)
{
