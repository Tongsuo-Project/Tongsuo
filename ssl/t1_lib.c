...
static const TLS_GROUP_INFO nid_list[] = {
    ...
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for brainpoolP256r1 (31)*/
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for brainpoolP384r1 (32)*/
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for brainpoolP512r1 (33)*/
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for GC256A (34)*/
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for GC256B (35)*/
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for GC256C (36)*/
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for GC256D (37)*/
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for GC512A (38)*/
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for GC512B (39)*/
    {NID_undef, 80, TLS_CURVE_PRIME}, /*left for GC512C (40)*/
...
static const SIGALG_LOOKUP sigalg_lookup_tbl[] = {
    ...
    {"sm2sig_sm3", TLSEXT_SIGALG_sm2sig_sm3,
     NID_sm3, SSL_MD_SM3_IDX, NID_sm2, SSL_PKEY_SM2,
...
/* get NID of signature algorithm and NID of hash algorithm */
int tls1_get_sig_and_hash(uint16_t sigalg, int *psig, int *phash)
{
...
int tls_choose_sigalg(SSL *s, int fatalerrs)
{
    ...
    if (SSL_IS_TLS13(s)) {
        ...
        if (SSL_USE_SIGALGS(s)) {
            ...
            if (s->s3->tmp.peer_sigalgs != NULL) {
                ...
                if (tls1_suiteb(s)) {
                    EC_KEY *ec;
                    ec = EVP_PKEY_get0_EC_KEY(s->cert->pkeys[SSL_PKEY_ECC].privatekey);
                    curve = EC_GROUP_get_curve_name(EC_KEY_get0_group(ec));
                } else {
                    curve = -1;
                }
    ...
    if (dc_lu) {
        ...
        s->s3->tmp.cert = s->cert->dc_pkeys[sig_idx].parent;
...
int tls1_set_shared_dc_sigalgs(SSL *s)
{
    ...
    if (nmatch) {
        if ((salgs = OPENSSL_malloc(nmatch * sizeof(*salgs))) == NULL) {
            SSLerr(SSL_F_TLS1_SET_SHARED_DC_SIGALGS, ERR_R_MALLOC_FAILURE);
            return 0;
        }
        nmatch = tls12_shared_sigalgs(s, salgs, pref, preflen, allow, allowlen);
    } else {
...
int tls1_lookup_sigalg_by_pkey_and_hash(EVP_PKEY *pkey, int hash, int is_tls13,
                                        uint16_t *psigalg)
{
    int md_nid = hash;
    int def_nid, def_ret;
    ...
    for (i = 0, s = sigalg_lookup_tbl; i < OSSL_NELEM(sigalg_lookup_tbl);
        ...
        if (s->sig_idx != (int)sig_idx)
            continue;

        if (s->hash != NID_undef && s->hash != md_nid)
            continue;

#ifndef OPENSSL_NO_EC
        if (s->curve != curve_nid)
            continue;
#endif
        if (psigalg)
            *psigalg = s->sigalg;
