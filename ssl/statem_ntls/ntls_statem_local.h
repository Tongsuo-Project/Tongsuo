...
__owur int tls_get_message_header_ntls(SSL *s, int *mt);
__owur int tls_get_message_body_ntls(SSL *s, size_t *len);
__owur int dtls_get_message_ntls(SSL *s, int *mt, size_t *len);

/* Message construction and processing functions */
__owur int tls_process_initial_server_flight_ntls(SSL *s);
__owur MSG_PROCESS_RETURN tls_process_change_cipher_spec_ntls(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_finished_ntls(SSL *s, PACKET *pkt);
__owur int tls_construct_change_cipher_spec_ntls(SSL *s, WPACKET *pkt);


__owur int tls_construct_finished_ntls(SSL *s, WPACKET *pkt);
__owur WORK_STATE tls_finish_handshake_ntls(SSL *s, WORK_STATE wst, int clearbufs,
                                       int stop);
__owur WORK_STATE dtls_wait_for_dry_ntls(SSL *s);

/* some client-only functions */
__owur int tls_construct_client_hello_ntls(SSL *s, WPACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_server_hello_ntls(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_certificate_request_ntls(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_new_session_ticket_ntls(SSL *s, PACKET *pkt);
__owur int tls_process_cert_status_body_ntls(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_cert_status_ntls(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_server_done_ntls(SSL *s, PACKET *pkt);
__owur int tls_construct_cert_verify_ntls(SSL *s, WPACKET *pkt);
__owur WORK_STATE tls_prepare_client_certificate_ntls(SSL *s, WORK_STATE wst);
__owur int tls_construct_client_certificate_ntls(SSL *s, WPACKET *pkt);
__owur int ssl_do_client_cert_cb_ntls(SSL *s, X509 **px509, EVP_PKEY **ppkey);
__owur int tls_construct_client_key_exchange_ntls(SSL *s, WPACKET *pkt);
__owur int tls_client_key_exchange_post_work_ntls(SSL *s);
__owur int tls_construct_cert_status_body_ntls(SSL *s, WPACKET *pkt);
__owur int tls_construct_cert_status_ntls(SSL *s, WPACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_key_exchange_ntls(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_server_certificate_ntls(SSL *s, PACKET *pkt);
__owur WORK_STATE tls_post_process_server_certificate_ntls(SSL *s, WORK_STATE wst);
__owur int ssl3_check_cert_and_algorithm_ntls(SSL *s);
#ifndef OPENSSL_NO_NEXTPROTONEG
__owur int tls_construct_next_proto_ntls(SSL *s, WPACKET *pkt);
#endif
__owur MSG_PROCESS_RETURN tls_process_hello_req_ntls(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN dtls_process_hello_verify_ntls(SSL *s, PACKET *pkt);
__owur int tls_construct_end_of_early_data_ntls(SSL *s, WPACKET *pkt);

/* some server-only functions */
__owur MSG_PROCESS_RETURN tls_process_client_hello_ntls(SSL *s, PACKET *pkt);
__owur WORK_STATE tls_post_process_client_hello_ntls(SSL *s, WORK_STATE wst);
__owur int tls_construct_server_hello_ntls(SSL *s, WPACKET *pkt);
__owur int dtls_construct_hello_verify_request_ntls(SSL *s, WPACKET *pkt);
__owur int tls_construct_server_certificate_ntls(SSL *s, WPACKET *pkt);
__owur int tls_construct_server_key_exchange_ntls(SSL *s, WPACKET *pkt);
__owur int tls_construct_certificate_request_ntls(SSL *s, WPACKET *pkt);
__owur int tls_construct_server_done_ntls(SSL *s, WPACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_client_certificate_ntls(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_client_key_exchange_ntls(SSL *s, PACKET *pkt);
__owur WORK_STATE tls_post_process_client_key_exchange_ntls(SSL *s, WORK_STATE wst);
__owur MSG_PROCESS_RETURN tls_process_cert_verify_ntls(SSL *s, PACKET *pkt);
#ifndef OPENSSL_NO_NEXTPROTONEG
__owur MSG_PROCESS_RETURN tls_process_next_proto_ntls(SSL *s, PACKET *pkt);
#endif
__owur int tls_construct_new_session_ticket_ntls(SSL *s, WPACKET *pkt);
...
__owur int tls_validate_all_contexts_ntls(SSL *s, unsigned int thisctx,
                                     RAW_EXTENSION *exts);
__owur int extension_is_relevant_ntls(SSL *s, unsigned int extctx,
                                 unsigned int thisctx);
__owur int tls_collect_extensions_ntls(SSL *s, PACKET *packet, unsigned int context,
                                  RAW_EXTENSION **res, size_t *len, int init);
__owur int tls_parse_extension_ntls(SSL *s, TLSEXT_INDEX idx, int context,
                               RAW_EXTENSION *exts,  X509 *x, size_t chainidx);
__owur int tls_parse_all_extensions_ntls(SSL *s, int context, RAW_EXTENSION *exts,
                                    X509 *x, size_t chainidx, int fin);
__owur int should_add_extension_ntls(SSL *s, unsigned int extctx,
                                unsigned int thisctx, int max_version);
__owur int tls_construct_extensions_ntls(SSL *s, WPACKET *pkt, unsigned int context,
                                    X509 *x, size_t chainidx);

__owur int tls_psk_do_binder_ntls(SSL *s, const EVP_MD *md,
...
int tls_parse_ctos_key_share_ntls(SSL *s, PACKET *pkt, unsigned int context, X509 *x,
                             size_t chainidx);
int tls_parse_ctos_cookie_ntls(SSL *s, PACKET *pkt, unsigned int context, X509 *x,
                          size_t chainidx);
...
int tls_parse_ctos_psk_kex_modes_ntls(SSL *s, PACKET *pkt, unsigned int context,
                                 X509 *x, size_t chainidx);
int tls_parse_ctos_psk_ntls(SSL *s, PACKET *pkt, unsigned int context, X509 *x,
                       size_t chainidx);
...
EXT_RETURN tls_construct_stoc_alpn_ntls(SSL *s, WPACKET *pkt, unsigned int context,
                                   X509 *x, size_t chainidx);
#ifndef OPENSSL_NO_SRTP
EXT_RETURN tls_construct_stoc_use_srtp_ntls(SSL *s, WPACKET *pkt, unsigned int context,
                                X509 *x, size_t chainidx);
#endif
...
EXT_RETURN tls_construct_stoc_key_share_ntls(SSL *s, WPACKET *pkt,
                                        unsigned int context, X509 *x,
                                        size_t chainidx);
EXT_RETURN tls_construct_stoc_cookie_ntls(SSL *s, WPACKET *pkt, unsigned int context,
                                     X509 *x, size_t chainidx);

EXT_RETURN tls_construct_stoc_psk_ntls(SSL *s, WPACKET *pkt, unsigned int context,
                                  X509 *x, size_t chainidx);
...
EXT_RETURN tls_construct_ctos_alpn_ntls(SSL *s, WPACKET *pkt, unsigned int context,
                                   X509 *x, size_t chainidx);
#ifndef OPENSSL_NO_SRTP
EXT_RETURN tls_construct_ctos_use_srtp_ntls(SSL *s, WPACKET *pkt, unsigned int context,
                                       X509 *x, size_t chainidx);
#endif
...
__owur OSSL_HANDSHAKE_STATE SSL_get_state_ntls(const SSL *ssl);
