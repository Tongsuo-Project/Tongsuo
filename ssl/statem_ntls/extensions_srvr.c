...
# ifndef OPENSSL_NO_TLS1_3
    ...
    if (!PACKET_as_length_prefixed_2(pkt, &cookie)) {
        SSLfatal_ntls(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_COOKIE_NTLS,
                 SSL_R_LENGTH_MISMATCH);
        return 0;
    }
    ...
    if (hctx == NULL || pkey == NULL) {
        ...
        EVP_PKEY_free(pkey);
        SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PARSE_CTOS_COOKIE_NTLS,
    ...
    if (CRYPTO_memcmp(hmac, mdin, SHA256_DIGEST_LENGTH) != 0) {
        SSLfatal_ntls(s, SSL_AD_ILLEGAL_PARAMETER, SSL_F_TLS_PARSE_CTOS_COOKIE_NTLS,
                       ...
                       size_t chainidx)
{
    ...
    if (!PACKET_get_length_prefixed_2(pkt, &identities)) {
        SSLfatal_ntls(s, SSL_AD_DECODE_ERROR,
                 SSL_F_TLS_PARSE_CTOS_PSK_NTLS, SSL_R_BAD_EXTENSION);
        return 0;
    }
    ...
    for (id = 0; PACKET_remaining(&identities) != 0; id++) {
        ...
        if (sess != NULL) {
            ...
            if (sesstmp == NULL) {
                SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR,
                         SSL_F_TLS_PARSE_CTOS_PSK_NTLS, ERR_R_INTERNAL_ERROR);
                return 0;
            }
            ...
            if (ret == SSL_TICKET_EMPTY) {
                SSLfatal_ntls(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_PSK_NTLS,
                         SSL_R_BAD_EXTENSION);
                return 0;
            }
...
# ifndef OPENSSL_NO_TLS1_3
    ...
    if (s->ctx->gen_stateless_cookie_cb == NULL) {
        SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_STOC_COOKIE_NTLS,
                                  ...
                                  X509 *x, size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_psk)
            ...
            || !WPACKET_close(pkt)) {
        SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR,
                 SSL_F_TLS_CONSTRUCT_STOC_PSK_NTLS, ERR_R_INTERNAL_ERROR);
        return EXT_RETURN_FAIL;
    }
