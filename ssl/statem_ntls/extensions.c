...
# ifndef OPENSSL_NO_EC
    {
        ...
        NULL, tls_parse_ctos_ec_pt_formats_ntls, tls_parse_stoc_ec_pt_formats_ntls,
...
int tls_validate_all_contexts_ntls(SSL *s, unsigned int thisctx, RAW_EXTENSION *exts)
{
 ...
 */
int extension_is_relevant_ntls(SSL *s, unsigned int extctx, unsigned int thisctx)
{
    int is_tls13;
 ...
 */
int tls_collect_extensions_ntls(SSL *s, PACKET *packet, unsigned int context,
                           RAW_EXTENSION **res, size_t *len, int init)
{
    ...
    *res = NULL;
    ...
    if (raw_extensions == NULL) {
        SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_COLLECT_EXTENSIONS_NTLS,
    ...
    while (PACKET_remaining(&extensions) > 0) {
        ...
        if (!PACKET_get_net_2(&extensions, &type) ||
            ...
            SSLfatal_ntls(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_COLLECT_EXTENSIONS_NTLS,
                     SSL_R_BAD_EXTENSION);
            goto err;
        }
        /*
        ...
        if (!verify_extension(s, context, type, exts, raw_extensions, &thisex)
            ...
            SSLfatal_ntls(s, SSL_AD_ILLEGAL_PARAMETER, SSL_F_TLS_COLLECT_EXTENSIONS_NTLS,
                     SSL_R_BAD_EXTENSION);
            goto err;
        }
...
# endif
            ...
            SSLfatal_ntls(s, SSL_AD_UNSUPPORTED_EXTENSION,
                     SSL_F_TLS_COLLECT_EXTENSIONS_NTLS, SSL_R_UNSOLICITED_EXTENSION);
            goto err;
        }
        if (thisex != NULL) {
 ...
 */
int tls_parse_extension_ntls(SSL *s, TLSEXT_INDEX idx, int context,
 ...
 */
int tls_parse_all_extensions_ntls(SSL *s, int context, RAW_EXTENSION *exts, X509 *x,
...
int should_add_extension_ntls(SSL *s, unsigned int extctx, unsigned int thisctx,
                         int max_version)
{
 ...
 */
int tls_construct_extensions_ntls(SSL *s, WPACKET *pkt, unsigned int context,
                             X509 *x, size_t chainidx)
{
    size_t i;
    ...
    if (!WPACKET_start_sub_packet_u16(pkt)
        ...
        SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_EXTENSIONS_NTLS,
...
static int final_renegotiate(SSL *s, unsigned int context, int sent)
{
    if (!s->server) {
        ...
        if (!(s->options & SSL_OP_LEGACY_SERVER_CONNECT)
                ...
                && !sent) {
            SSLfatal_ntls(s, SSL_AD_HANDSHAKE_FAILURE, SSL_F_FINAL_RENEGOTIATE,
...
static int final_server_name(SSL *s, unsigned int context, int sent)
{
    ...
    if (!ossl_assert(s->ctx != NULL) || !ossl_assert(s->session_ctx != NULL)) {
        SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, SSL_F_FINAL_SERVER_NAME,
                 ERR_R_INTERNAL_ERROR);
        return 0;
    }
    ...
    case SSL_TLSEXT_ERR_ALERT_FATAL:
        SSLfatal_ntls(s, altmp, SSL_F_FINAL_SERVER_NAME, SSL_R_CALLBACK_FAILED);
        return 0;
...
# endif
    ...
    if (!ossl_assert(hashsizei >= 0)) {
        SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PSK_DO_BINDER_NTLS,
                 ERR_R_INTERNAL_ERROR);
        goto err;
    }
    ...
    if (sign) {
        ...
        if (!ret)
            SSLfatal_ntls(s, SSL_AD_ILLEGAL_PARAMETER, SSL_F_TLS_PSK_DO_BINDER_NTLS,
...
static int final_early_data(SSL *s, unsigned int context, int sent)
{
    ...
    if (!s->server) {
        if (context == SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS
             ...
             * ALPN)
             */
            SSLfatal_ntls(s, SSL_AD_ILLEGAL_PARAMETER, SSL_F_FINAL_EARLY_DATA,
...
static int final_maxfragmentlen(SSL *s, unsigned int context, int sent)
{
    ...
    if (s->server && s->hit && USE_MAX_FRAGMENT_LENGTH_EXT(s->session)
            && !sent ) {
        SSLfatal_ntls(s, SSL_AD_MISSING_EXTENSION, SSL_F_FINAL_MAXFRAGMENTLEN,
                 SSL_R_BAD_EXTENSION);
        return 0;
    }
