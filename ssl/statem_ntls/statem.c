...
static void init_read_state_machine_ntls(SSL *s);
static SUB_STATE_RETURN read_state_machine_ntls(SSL *s);
static void init_write_state_machine_ntls(SSL *s);
static SUB_STATE_RETURN write_state_machine_ntls(SSL *s);

OSSL_HANDSHAKE_STATE SSL_get_state_ntls(const SSL *ssl)
{
                       ...
                       int line)
{
    ...
    /* We shouldn't call SSLfatal_ntls() twice. Once is enough */
...
# define check_fatal(s, f) \
    do { \
        if (!ossl_assert((s)->statem.in_init \
                         && (s)->statem.state == MSG_FLOW_ERROR)) \
            SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, (f), \
                     SSL_R_MISSING_FATAL); \
    } while (0)
...
int ossl_statem_skip_early_data_ntls(SSL *s)
{
 ...
 * or similar.
 */
void ossl_statem_check_finish_init_ntls(SSL *s, int sending)
{
    if (sending == -1) {
...
# endif
    ...
    while (st->state != MSG_FLOW_FINISHED) {
        if (st->state == MSG_FLOW_READING) {
            ...
            /* Error */
            check_fatal(s, SSL_F_STATE_MACHINE_NTLS);
            SSLerr(SSL_F_STATE_MACHINE_NTLS, ERR_R_SHOULD_NOT_HAVE_BEEN_CALLED);
            goto end;
        }
    }
    ...
    while (1) {
        ...
        case READ_STATE_HEADER:
            ...
            if (s->s3->tmp.message_size > max_message_size(s)) {
                SSLfatal_ntls(s, SSL_AD_ILLEGAL_PARAMETER, SSL_F_READ_STATE_MACHINE_NTLS,
        ...
        case READ_STATE_POST_PROCESS:
            ...
            case WORK_ERROR:
                check_fatal(s, SSL_F_READ_STATE_MACHINE_NTLS);
                /* Fall through */
            case WORK_MORE_A:
            case WORK_MORE_B:
            case WORK_MORE_C:
    ...
    while (1) {
        ...
        case WRITE_STATE_TRANSITION:
            ...
            case WRITE_TRAN_ERROR:
                check_fatal(s, SSL_F_WRITE_STATE_MACHINE_NTLS);
                return SUB_STATE_ERROR;
            }
            break;
 ...
 */
int ossl_statem_app_data_allowed_ntls(SSL *s)
{
    OSSL_STATEM *st = &s->statem;
 ...
 */
int ossl_statem_export_allowed_ntls(SSL *s)
{
 ...
 * or 0 if otherwise.
 */
int ossl_statem_export_early_allowed_ntls(SSL *s)
{
    /*
