                     ...
                     size_t chainidx)
{
    ...
    if ((context & (SSL_EXT_TLS1_2_SERVER_HELLO
        ...
        if ((meth->ext_flags & SSL_EXT_FLAG_SENT) == 0) {
            SSLfatal_ntls(s, TLS1_AD_UNSUPPORTED_EXTENSION, SSL_F_CUSTOM_EXT_PARSE_NTLS,
                     SSL_R_BAD_EXTENSION);
            return 0;
        }
    }
    ...
    if (meth->parse_cb(s, ext_type, context, ext_data, ext_size, x, chainidx,
                       &al, meth->parse_arg) <= 0) {
        SSLfatal_ntls(s, al, SSL_F_CUSTOM_EXT_PARSE_NTLS, SSL_R_BAD_EXTENSION);
        return 0;
    }
                   ...
                   int maxversion)
{
    ...
    for (i = 0; i < exts->meths_count; i++) {
        ...
        if (meth->add_cb != NULL) {
            ...
            if (cb_retval < 0) {
                SSLfatal_ntls(s, al, SSL_F_CUSTOM_EXT_ADD_NTLS, SSL_R_CALLBACK_FAILED);
                return 0;       /* error */
            }
            if (cb_retval == 0)
        ...
        if (!WPACKET_put_bytes_u16(pkt, meth->ext_type)
                ...
                || !WPACKET_close(pkt)) {
            SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, SSL_F_CUSTOM_EXT_ADD_NTLS,
