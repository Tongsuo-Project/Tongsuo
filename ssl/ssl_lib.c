...
void ssl_update_cache(SSL *s, int mode)
{
    ...
    if (s->global_session_state == SSL_SESS_STAT_WAITING_NEW) {
        i = s->session_ctx->session_cache_mode;
        s->global_session_state = SSL_SESS_STAT_WAITING_NONE;
        goto new_session_reentry;
    }
#endif
...
#ifndef OPENSSL_NO_GLOBAL_SESSION_CACHE
            ret = s->session_ctx->new_session_cb(s, s->session);
            if(ret == -1) {
                SSL_SESSION_free(s->session);
                s->global_session_state = SSL_SESS_STAT_WAITING_NEW;
                return;
            } else if (!ret) {
                SSL_SESSION_free(s->session);
            }
...
#endif
#ifndef OPENSSL_NO_GLOBAL_SESSION_CACHE
    if (SSL_want_sess_global_lookup(s))
        return SSL_ERROR_WANT_SESSION_RESULT;
#endif
...
SSL_CTX *SSL_CTX_dup(SSL_CTX *ctx)
{
    ...
    if ((ret->md5 = EVP_get_digestbyname("ssl3-md5")) == NULL) {
        SSLerr(SSL_F_SSL_CTX_DUP, SSL_R_UNABLE_TO_LOAD_SSL3_MD5_ROUTINES);
        goto err2;
    }
    if ((ret->sha1 = EVP_get_digestbyname("ssl3-sha1")) == NULL) {
        SSLerr(SSL_F_SSL_CTX_DUP, SSL_R_UNABLE_TO_LOAD_SSL3_SHA1_ROUTINES);
        goto err2;
    }
...
# endif
    ...
    /* don't set sni callback here */
    ret->ext.servername_cb = 0;
    ret->ext.servername_arg = NULL;
    /* dup RFC4507 ticket keys */
    memcpy(ret->ext.tick_key_name, ctx->ext.tick_key_name, TLSEXT_KEYNAME_LENGTH);
    ret->ext.secure = OPENSSL_zalloc(sizeof(SSL_CTX_EXT_SECURE));
    if (ret->ext.secure == NULL)
        goto err;

    memcpy(ret->ext.secure->tick_hmac_key, ctx->ext.secure->tick_hmac_key, TLSEXT_TICK_KEY_LENGTH);
    memcpy(ret->ext.secure->tick_aes_key, ctx->ext.secure->tick_aes_key, TLSEXT_TICK_KEY_LENGTH);
...
# endif
    ...
    return (ret);
 err:
    SSLerr(SSL_F_SSL_CTX_DUP, ERR_R_MALLOC_FAILURE);
 err2:
    if (ret != NULL)
        SSL_CTX_free(ret);
    return (NULL);
...
#ifdef SSL_DEBUG
    if (EVP_MD_CTX_type(hdgst) != NID_sm3) {
        fprintf(stderr, "handshake dgst type: %d, %s\n",
                EVP_MD_CTX_type(hdgst), OBJ_nid2ln(EVP_MD_CTX_type(hdgst)));
    }
#endif
...
{
    if (s != NULL)
        s->global_session_result = result;
}
#endif
...
int SSL_get_desc_and_level(SSL *s, int *level, int *desc)
{
    ...
    if (level != NULL)
        *level = s->s3->alert_level;

    if (desc != NULL) {
        if (s->s3->alert_level == SSL3_AL_WARNING)
            *desc = s->s3->warn_alert;
        else if (s->s3->alert_level == SSL3_AL_FATAL)
            *desc = s->s3->fatal_alert;
    }
