 ...
 *
 * Returns:
 *   -3: want asynchronous session lookup by lua
 *   -2: want global session lookup
 *   -1: fatal error
 *    0: no session found
...
# ifndef OPENSSL_NO_GLOBAL_SESSION_CACHE
        if (s->global_session_state) {
            ret = s->global_session_result;
            try_session_cache = s->global_session_recover_try;
...
# ifndef OPENSSL_NO_SESSION_LOOKUP
    if (ret == SSL_magic_pending_session_ptr())
        return -3; /* Retry later */
# endif

# ifndef OPENSSL_NO_GLOBAL_SESSION_CACHE
    if (ret == (SSL_SESSION *)-1) {
        s->global_session_recover_try = try_session_cache;
        /* -2 means again */
        return -2;
    }
