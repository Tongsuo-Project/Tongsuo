...
static void *ml_dsa_gen(void * genctx, OSSL_CALLBACK * osslcb, void * cbarg)
{
    ...
    if (gctx->seed_len != 0)
        seedbuf = gctx->seed;
    if (crypto_sign_keypair(key->pubkey, key->privkey, seedbuf)) {
...
static int ml_dsa_get_params(void *keydata, OSSL_PARAM params[])
{
    ...
    if ((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_PUB_KEY)) != NULL) {
        if (!OSSL_PARAM_set_octet_string(p, key->pubkey, ML_DSA_PUBLICKEYBYTES))
            return 0;
    }

    if ((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_PRIV_KEY)) != NULL) {
        if (!OSSL_PARAM_set_octet_string(p, key->privkey, ML_DSA_SECRETKEYBYTES))
            return 0;
    }
...
static int ml_dsa_match(const void *keydata1, const void *keydata2, int selection)
{
    ...
    return memcmp(key1, key2, sizeof(ML_DSA_KEY));
}
...
const OSSL_DISPATCH ossl_ml_dsa_keymgmt_functions[] = {
