...
/*
 * This file uses the low level wbsm4_xiao_stkey functions (which are deprecated for
 * non-internal use) in order to implement provider wbsm4_xiao_stkey ciphers.
 */
...
static int wbsm4_xiao_stkey_gcm_initkey(PROV_GCM_CTX *ctx, const unsigned char *key,
                           size_t keylen)
{
    PROV_WBSM4_XIAO_STKEY_GCM_CTX *actx = (PROV_WBSM4_XIAO_STKEY_GCM_CTX *)ctx;
    wbsm4_xiao_stkey_context *ks = &actx->ks.ks;

    ctx->ks = ks;

    wbsm4_set_key(key, ks, sizeof(wbsm4_xiao_stkey_context));
    CRYPTO_gcm128_init(&ctx->gcm, ks, (block128_f)wbsm4_xiao_stkey_encrypt);
...
static int generic_wbsm4_xiao_stkey_gcm_cipher_update(PROV_GCM_CTX *ctx,
...
static const PROV_GCM_HW wbsm4_xiao_stkey_gcm = {
    wbsm4_xiao_stkey_gcm_initkey,
    ossl_gcm_setiv,
    ossl_gcm_aad_update,
    generic_wbsm4_xiao_stkey_gcm_cipher_update,
...
{
    return &wbsm4_xiao_stkey_gcm;
}
#endif

#ifndef OPENSSL_NO_WBSM4_JIN_STKEY
static int wbsm4_jin_stkey_gcm_initkey(PROV_GCM_CTX *ctx, const unsigned char *key,
                           size_t keylen)
{
    PROV_WBSM4_JIN_STKEY_GCM_CTX *actx = (PROV_WBSM4_JIN_STKEY_GCM_CTX *)ctx;
    wbsm4_jin_stkey_context *ks = &actx->ks.ks;

    ctx->ks = ks;

    wbsm4_set_key(key, ks, sizeof(wbsm4_jin_stkey_context));
    CRYPTO_gcm128_init(&ctx->gcm, ks, (block128_f)wbsm4_jin_stkey_encrypt);
    ctx->ctr = (ctr128_f)NULL;
...
static int generic_wbsm4_jin_stkey_gcm_cipher_update(PROV_GCM_CTX *ctx,
                                         const unsigned char *in,
                                         size_t len, unsigned char *out)
{
    if (ctx->enc) {
...
static const PROV_GCM_HW wbsm4_jin_stkey_gcm = {
    wbsm4_jin_stkey_gcm_initkey,
    ossl_gcm_setiv,
    ossl_gcm_aad_update,
    generic_wbsm4_jin_stkey_gcm_cipher_update,
    ossl_gcm_cipher_final,
    ossl_gcm_one_shot
};
