...
static int test_wbsm4_standard(void)
{
    ...
    if (!TEST_ptr(wb = OPENSSL_malloc(whitebox_len))){
        OPENSSL_free(wb);
        return 0;
    }
    wbsm4_gen(key,wb,&whitebox_len);
...
static int test_wbsm4_random_gen_tables(void){
    ...
    if(!TEST_int_eq(RAND_bytes(key,16), 1)){
        return 0;
    }

    wbsm4_gen(key,wb1,&whitebox_len1);
    if (!TEST_ptr(wb1 = OPENSSL_malloc(whitebox_len1))){
        OPENSSL_free(wb1);
        return 0;
    }
    wbsm4_gen(key,wb2,&whitebox_len2);
    if (!TEST_ptr(wb2 = OPENSSL_malloc(whitebox_len2))){
        OPENSSL_free(wb2);
        return 0;
    }
    wbsm4_gen(key,wb1,&whitebox_len1);
...
static int test_wbsm4_random_key_and_input(void){
    ...
    if(!TEST_int_eq(RAND_bytes(k,SM4_BLOCK_SIZE), 1)){
        return 0;
    }
