...
static int test_wbsm4_with_whitebox(void){
    ...
    read_wb = (unsigned char*)OPENSSL_malloc(file_size);
    read = fread(read_wb, sizeof(unsigned char), file_size, fp);
    fclose(fp);
    if (read != file_size) {
        printf("ERROR: The file was read incompletely\n");
