...
static int test_EVP_sm4_gcm(void)
{
    int ret = 0;
    unsigned char tag[16];
    unsigned char buf[1024];
    ...
    if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new())
        || !TEST_true(EVP_EncryptInit_ex(ctx, EVP_sm4_gcm(), NULL, key, iv))
        || !TEST_true(EVP_EncryptUpdate(ctx, NULL, &outlen, aad, sizeof(aad)))
        || !TEST_true(EVP_EncryptUpdate(ctx, buf, &outlen, plaintext,
                                        sizeof(plaintext)))
        || !TEST_true(EVP_EncryptFinal_ex(ctx, buf + outlen, &tmplen)))
        goto end;

    outlen += tmplen;

    if (!TEST_int_eq(outlen, sizeof(ciphertext))
        || !TEST_mem_eq(buf, outlen, ciphertext, sizeof(ciphertext)))
        goto end;

    if (!TEST_true(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, sizeof(tag),
                                       tag))
...
static int test_EVP_sm4_ccm(void)
{
    int ret = 0;
    unsigned char tag[16];
    unsigned char buf[1024];
    ...
    if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new())
        || !TEST_true(EVP_EncryptInit_ex(ctx, EVP_sm4_ccm(), NULL, NULL, NULL))
        ...
        || !TEST_true(EVP_EncryptUpdate(ctx, buf, &outlen, plaintext,
                                        sizeof(plaintext)))
        || !TEST_true(EVP_EncryptFinal_ex(ctx, NULL, &tmplen)))
        goto end;
