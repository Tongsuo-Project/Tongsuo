...
// {
//     // 测试密钥
//     static const uint8_t k[WB_SM4_KEY_SIZE] = {
//         0x78, 0x3b, 0xd7, 0x63, 0x47, 0xaa, 0x6b, 0xfe,
//         0x47, 0x05, 0xeb, 0xc0, 0x60, 0x4a, 0x7b, 0x0f
//     };

//     // 测试输入
//     static const uint8_t input[WB_SM4_BLOCK_SIZE] = {
//         0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
//         0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff
//     };

//     // 预期的加密结果
//     static const uint8_t expected[WB_SM4_BLOCK_SIZE] = {
//         0x32, 0x1a, 0xfa, 0xbb, 0x83, 0x47, 0xb5, 0xff,
//         0x94, 0x07, 0x78, 0xb4, 0xf6, 0xdf, 0x1b, 0x37
//     };
//     int ret = 0;
//     WB_SM4_Tables *tables = NULL;
//     uint8_t block[WB_SM4_BLOCK_SIZE];
//     uint8_t encrypted[WB_SM4_BLOCK_SIZE]; // 加密结果存储
//     uint8_t decrypted[WB_SM4_BLOCK_SIZE]; // 解密结果存储
//     // 创建白盒表
//     tables = malloc(sizeof(WB_SM4_Tables));
//     if (!TEST_ptr(tables))
//         goto err;
...
//     // 执行加密
//     memcpy(block, input, WB_SM4_BLOCK_SIZE);
//     Nonlinearwbsm4_encrypt(block, encrypted, tables)
//     // 检查加密结果
//     if (!TEST_mem_eq(encrypted, WB_SM4_BLOCK_SIZE, expected, WB_SM4_BLOCK_SIZE))
//         goto err;
//     // 执行解密
//     Nonlinearwbsm4_decrypt(encrypted, decrypted, tables)
//     // 检查解密结果是否与原始输入一致
//     if (!TEST_mem_eq(decrypted, WB_SM4_BLOCK_SIZE, input, WB_SM4_BLOCK_SIZE))
//         goto err;
