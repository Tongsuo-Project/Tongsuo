...
/*
 * A 64-bit implementation of the SM2 P-256 elliptic curve point multiplication
 *
...
/*
 * The underlying field. SM2 P-256 operates over GF(2^256-2^224-2^96+2^64-1). We
...
/*
 * These are the parameters of SM2 P-256, taken from GM/T 0003.5-2012, page 1. These
 * values are big-endian.
 */
static const felem_bytearray sm2p256_curve_params[5] = {
...
/* zero105 is (2^41 * p) mod p = 0 mod p*/
...
/* zero107 is (2^43 * p) mod p = 0 mod p*/
...
{
    /* construct a zero num equals to 
     * (2^262+2^230+2^198+2^166+2^134+2^103+2^71+2^39+192) * p 
     */
    static const limb two70m39m7m6 =
        (((limb) 1) << 70) - (((limb) 1) << 39) - (((limb) 1) << 7) - (((limb) 1) << 6);
    static const limb two70m40p38 = (((limb) 1) << 70) - (((limb) 1) << 40) + (((limb) 1) << 38);
    static const limb two70m38m7 = (((limb) 1) << 70) - (((limb) 1) << 38) - (((limb) 1) << 7);
...
/* zerop is equal to p */
static const felem zerop = { two64m0, two64m32, two64m0, two64m32m0 };
...
static void felem_shrink(smallfelem out, const felem in)
{
    ...
    u64 a, b, mask;
    u64 high, low;
    static const u64 kPrime3Test = 0x7ffffffefffffffful; /* 2^63 - 2^32 - 1 */

    /* Carry 2->3 */
    tmp[3] = zerop[3] + in[3] + ((u64)(in[2] >> 64));
    /* tmp[3] < 2^110 */

    tmp[2] = zerop[2] + (u64)in[2];
    tmp[0] = zerop[0] + in[0];
    tmp[1] = zerop[1] + in[1];
 ...
 *   a^{p-2} = a^{-1} (mod p)
 * 
 * Use Addition chain to obtain a faster algorithm with fewer operations (255S + 14M).
 * You can get more information from https://jeit.ac.cn/cn/article/doi/10.11999/JEIT211049 .
 */
...
const EC_METHOD *EC_GFp_sm2p256_method(void)
{
    static const EC_METHOD ret = {
        ...
        ossl_ec_GFp_sm2p256_group_init,
        ossl_ec_GFp_simple_group_finish,
        ossl_ec_GFp_simple_group_clear_finish,
        ossl_ec_GFp_nist_group_copy,
        ossl_ec_GFp_sm2p256_group_set_curve,
        ...
        ossl_ec_GFp_sm2p256_point_get_affine_coordinates,
...
static SM2P256_PRE_COMP *sm2p256_pre_comp_new(void)
{
...
void EC_sm2p256_pre_comp_free(SM2P256_PRE_COMP *pre)
{
    ...
    REF_PRINT_COUNT("EC_sm2p256", pre);
    if (i > 0)
        return;
    REF_ASSERT_ISNT(i < 0);
                                         ...
                                         BN_CTX *ctx)
{
    ...
    if (curve_b == NULL)
        goto err;
    BN_bin2bn(sm2p256_curve_params[0], sizeof(felem_bytearray), curve_p);
    BN_bin2bn(sm2p256_curve_params[1], sizeof(felem_bytearray), curve_a);
    BN_bin2bn(sm2p256_curve_params[2], sizeof(felem_bytearray), curve_b);
...
int ossl_ec_GFp_sm2p256_precompute_mult(EC_GROUP *group, BN_CTX *ctx)
{
    ...
    if (generator == NULL)
        goto err;
    BN_bin2bn(sm2p256_curve_params[3], sizeof(felem_bytearray), x);
    BN_bin2bn(sm2p256_curve_params[4], sizeof(felem_bytearray), y);
