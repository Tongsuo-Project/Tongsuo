...
//测试函数
//void printNonlinear32(Nonlinear32* nl32) {
//    for (int i = 0; i < 256; i++) {
//        // 打印拼接后的 32-bit 映射值
//        printf("Mapping[%d]: 0x%02X%02X%02X%02X\n", i,
//            nl32->n8_1.mapping[i],
//            nl32->n8_2.mapping[i],
//            nl32->n8_3.mapping[i],
//            nl32->n8_4.mapping[i]);
...
//void printNonlinear8(Nonlinear8* nl8) {
//    for (int i = 0; i < 256; i++) {
//        printf("Mapping[%d]: 0x%02X\n", i, nl8->mapping[i]);
//    }
//}
//void printNonlinear32WithDetails(Nonlinear8 n1, Nonlinear8 n2, Nonlinear8 n3, Nonlinear8 n4, Nonlinear32* n32) {
//    for (int i = 0; i < 256; i++) {
//        // 打印拼接前的四个 8 位映射
//        printf("Mapping[%d]: n1 = 0x%02X, n2 = 0x%02X, n3 = 0x%02X, n4 = 0x%02X -> ",
//            i, n1.mapping[i], n2.mapping[i], n3.mapping[i], n4.mapping[i]);
//        // 拼接后的 32 位值
//        uint32_t result = (n1.mapping[i] << 24) | (n2.mapping[i] << 16) | (n3.mapping[i] << 8) | n4.mapping[i];
//        printf("n1 << 24 | n2 << 16 | n3 << 8 | n4 = 0x%08X\n", result);
//        // 打印 Nonlinear32 中的拼接后的值
//        printf("Result in Nonlinear32: 0x%08X\n", n32->n8_1.mapping[i] << 24 | n32->n8_2.mapping[i] << 16 | n32->n8_3.mapping[i] << 8 | n32->n8_4.mapping[i]);
//    }
//}
// 打印非线性双射对
//void printNonlinearPair8(Nonlinear8* nl, Nonlinear8* nl_inv) {
//    for (int i = 0; i < 256; i++) {
//        // 显示映射和逆映射
//        printf("Mapping: %u -> %u, Inverse Mapping: %u -> %u\n",
//            i, nl->mapping[i], nl_inv->mapping[nl->mapping[i]], nl->mapping[i]);
//    }
//}
