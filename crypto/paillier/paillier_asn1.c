...
} ASN1_SEQUENCE_END_cb(PAILLIER_KEY, PAILLIER_PrivateKey)

ASN1_SEQUENCE_cb(PAILLIER_PublicKey, paillier_cb) = {
        ASN1_SIMPLE(PAILLIER_KEY, n, BIGNUM),
        ASN1_SIMPLE(PAILLIER_KEY, g, CBIGNUM),
} ASN1_SEQUENCE_END_cb(PAILLIER_KEY, PAILLIER_PublicKey)
...
RSA *PAILLIER_PublicKey_dup(const PAILLIER_KEY *pail)
{
    return ASN1_item_dup(ASN1_ITEM_rptr(PAILLIER_PublicKey), pail);
}

RSA *PAILLIER_PrivateKey_dup(const PAILLIER_KEY *pail)
{
    return ASN1_item_dup(ASN1_ITEM_rptr(PAILLIER_PrivateKey), pail);
}
