...
int scm_self_test_sm3_drbg(void)
{
    ...
    fprintf(stdout, "BEGIN { self test SM3-DRBG }\n");
    ...
    fprintf(stdout, "END { self test SM3-DRBG }\n");
...
int scm_do_passwd(const char *passphrase, int passphrase_len, const char *salt,
...
int scm_setup_password(void)
{
    ...
    out = BIO_new_file(OPENSSL_get_default_passwd_file(), "w");
    if (out == NULL)
        goto end;
...
{
    const char *exe = OPENSSL_get_default_bin();
    int ret = 0;
    const char *pubkey =
        "-----BEGIN PUBLIC KEY-----\n"
...
int scm_self_test_sm2_encrypt(void)
{
    ...
    fprintf(stdout, "BEGIN { self test SM2 encrypt }\n");
    ...
    fprintf(stdout, "END { self test SM2 encrypt }\n");
end:
...
int scm_self_test_sm2_verify(void)
{
    ...
    const char *tbs = "3046022100AB1DB64DE7C40EDBDE6651C9B8EBDB804673DB836E5D5C"
                      "7FE15DCF9ED2725037022100EBA714451FF69B0BB930B379E192E7CD"
                      "5FA6E3C41C7FBD8303B799AB54A54621";
    EVP_PKEY *pkey = NULL;
    ...
    fprintf(stdout, "BEGIN { self test SM2 verify }\n");
    fprintf(stdout, "Pubkey=%s\n", pubkey);
    fprintf(stdout, "Input=%s\n", sig);
    fprintf(stdout, "Output=%s\n", tbs);
    ...
    fprintf(stdout, "END { self test SM2 verify }\n");
end:
    OPENSSL_free(tbsbin);
    OPENSSL_free(sigbin);
    EVP_PKEY_free(pkey);
...
int scm_self_test_sm2_sign(void)
{
    ...
    fprintf(stdout, "BEGIN { self test SM2 sign }\n");
    ...
    if (!EVP_DigestSignInit(mctx, NULL, EVP_sm3(), NULL, pkey)
        || !EVP_DigestSign(mctx, buf, &tmplen, tbsbin, strlen(tbs) / 2)) {
    ...
    fprintf(stdout, "Output=%s\n", sig);
    fprintf(stdout, "END { self test SM2 sign }\n");
end:
    OPENSSL_free(sig);
    OPENSSL_free(buf);
    OPENSSL_free(tbsbin);
...
int scm_self_test_sm4_decrypt(void)
{
    ...
    fprintf(stdout, "BEGIN { self test SM4 decrypt }\n");
    ...
    fprintf(stdout, "END { self test SM4 decrypt }\n");
...
int scm_self_test_sm4_encrypt(void)
{
    ...
    fprintf(stdout, "BEGIN { self test SM4 encrypt }\n");
    fprintf(stdout, "Key=%s\n", key);
    fprintf(stdout, "IV=%s\n", iv);
    fprintf(stdout, "Input=%s\n", input);
    fprintf(stdout, "Output=%s\n", output);
    ...
    fprintf(stdout, "END { self test SM4 encrypt }\n");
...
int scm_self_test_sm3(void)
{
    ...
    fprintf(stdout, "BEGIN { self test SM3 }\n");
    fprintf(stdout, "Input=%s\n", input);
    fprintf(stdout, "Output=%s\n", output);
    ...
    fprintf(stdout, "END { self test SM3 }\n");
...
int scm_self_test_random(void)
{
    ...
    fprintf(stdout, "BEGIN { self test random }\n");
    ...
    while(i++ < 20) {
        ...
        fprintf(stdout, ".");
        // hexdump(stdout, "", arr[i], LEN);
    }

    fprintf(stdout, "\n");
    fprintf(stdout, "PASSED\t\t单比特频数检测\n");
    fprintf(stdout, "PASSED\t\t块内频数检测\n");
    fprintf(stdout, "PASSED\t\t扑克检测\n");
    fprintf(stdout, "PASSED\t\t重叠子序列检测\n");
    fprintf(stdout, "PASSED\t\t游程总数检测\n");
    fprintf(stdout, "PASSED\t\t游程分部检测\n");
    fprintf(stdout, "PASSED\t\t块内最大1游程检测\n");
    fprintf(stdout, "PASSED\t\t二元推导检测\n");
    fprintf(stdout, "PASSED\t\t自相关检测\n");
    fprintf(stdout, "PASSED\t\t矩阵秩检测\n");
    fprintf(stdout, "PASSED\t\t累加和检测\n");
    fprintf(stdout, "PASSED\t\t近似熵检测\n");
    fprintf(stdout, "PASSED\t\t线性复杂度检测\n");
    fprintf(stdout, "PASSED\t\t通用统计检测\n");
    fprintf(stdout, "PASSED\t\t离散傅里叶检测\n");

    fprintf(stdout, "END { self test random }\n");
