...
{
    EVP_WBSM4_WSISE_KEY *dat = EVP_C_DATA(EVP_WBSM4_WSISE_KEY,ctx);
    ...
    dat->block = (block128_f)wbsm4_wsise_encrypt;
    wbsm4_wsise_set_key(key, &dat->ks.ks);
    
    return 1;
}

static int wbsm4_wsise_cbc_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
...
static int wbsm4_wsise_cfb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
                          const unsigned char *in, size_t len)
{
    EVP_WBSM4_WSISE_KEY *dat = EVP_C_DATA(EVP_WBSM4_WSISE_KEY,ctx);
...
static int wbsm4_wsise_ecb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
                          const unsigned char *in, size_t len)
{
...
static int wbsm4_wsise_ofb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
                          const unsigned char *in, size_t len)
{
    EVP_WBSM4_WSISE_KEY *dat = EVP_C_DATA(EVP_WBSM4_WSISE_KEY,ctx);
    int num = EVP_CIPHER_CTX_get_num(ctx);
...
static int wbsm4_wsise_ctr_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
                          const unsigned char *in, size_t len)
{
...
static int wbsm4_wsise_ccm_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
                          const unsigned char *in, size_t len);
static int wbsm4_wsise_ccm_cleanup(EVP_CIPHER_CTX *c);
                        ...
                        const unsigned char *iv, int enc)
{
    ...
    if (key) {
        do {
            wbsm4_wsise_set_key(key, &gctx->ks);
...
static int wbsm4_wsise_gcm_tls_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
                              const unsigned char *in, size_t len)
{
    EVP_SM4_GCM_CTX *gctx = EVP_C_DATA(EVP_SM4_GCM_CTX,ctx);
    int rv = -1;
                          ...
                          const unsigned char *in, size_t len)
{
    ...
    if (gctx->tls_aad_len >= 0)
        return wbsm4_wsise_gcm_tls_cipher(ctx, out, in, len);
                        ...
                        const unsigned char *iv, int enc)
{
    ...
    if (key != NULL)
        do {
            wbsm4_wsise_set_key(key, &cctx->ks);
            CRYPTO_ccm128_init(&cctx->ccm, cctx->M, cctx->L,
                               &cctx->ks, (block128_f)wbsm4_wsise_encrypt);
            cctx->str = NULL;
            cctx->key_set = 1;
        } while (0);
    if (iv != NULL) {
...
BLOCK_CIPHER_custom(NID_wbsm4_wsise, 1, 12, ccm, CCM,
                    EVP_CIPH_FLAG_AEAD_CIPHER | CUSTOM_FLAGS)
#endif
